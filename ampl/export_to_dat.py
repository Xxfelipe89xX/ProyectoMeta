...
def build_ampl_data(params, nodes, dist_matrix, speed_matrix):
    ...
    for i in range(n_ampl):
        for j in range(n_ampl):
            ...
            c_fixed[i][j] = (eff_d * eff_m *
                             (alpha * curb_w * d_m + beta * s_ms * s_ms * d_m)
                             / 3_600_000.0)
            c_load[i][j]  = (eff_d * eff_m * alpha * 1000.0 * d_m
                             / 3_600_000.0)
...
def write_dat(ampl_data, output_path, instance_name=''):
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f"# Nodes: {n} (depot_start + {len(ampl_data['customers'])} customers"
                f" + {len(ampl_data['stations'])} station copies + depot_end)\n\n")

        # --- Sets ---
        f.write(f"set NODES := {' '.join(str(i) for i in all_nodes)};\n\n")
        f.write(f"set CUSTOMERS := {' '.join(str(i) for i in ampl_data['customers'])};\n\n")
        f.write(f"set STATIONS := {' '.join(str(i) for i in ampl_data['stations'])};\n\n")
        f.write(f"set VEHICLES := {' '.join(str(i) for i in ampl_data['vehicles'])};\n\n")
